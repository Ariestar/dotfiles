...
"
    ...
    let host = (sys host)
    let mem = (sys mem)
    let uptime_str = $host.uptime
    let mem_used = ($mem.used | into string)
    let mem_total = ($mem.total | into string)
    
    print $"    ($yellow)󰍛($reset)  ($bold)User($reset)     ($env.USERNAME)@($host.hostname)"
    print $"    ($cyan)($reset)  ($bold)OS($reset)       ($host.name) ($host.os_version)"
    print $"    ($green)󰥔($reset)  ($bold)Uptime($reset)   ($uptime_str)"
    print $"    ($magenta)󰍛($reset)  ($bold)Memory($reset)   ($mem_used) / ($mem_total)"
    print $"    ($yellow)($reset)  ($bold)Shell($reset)    Nushell ((version).version)"
    print ""
}
